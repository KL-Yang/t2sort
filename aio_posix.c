 ...
 * */
void t2_aio_setup() {   /* do nothing */ }
void t2_aio_destroy() { /* do nothing */ }

static void
t2_aio_write(t2_aio_t *t2cb, int fd, void *buf, size_t count, off_t offset)
{
...
static void
t2_aio_read(t2_aio_t *t2cb, int fd, void *buf, size_t count, off_t offset)
{
    t2cb->paio.aio_fildes = fd;
    t2cb->paio.aio_buf    = buf;
    t2cb->paio.aio_nbytes = count;
...
static void t2_aio_wait(t2_aio_t *t2cb, int n)
{
